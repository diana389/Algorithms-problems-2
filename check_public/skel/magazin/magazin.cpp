...
// dfs array, initially set to -1
vector<int> dfs(((int)1e5 + 5), -1);

// array of indexes in dfs array, initially set to -1
vector<int> dfs_indexes(((int)1e5 + 5), -1);

// start time array
vector<int> start(((int)1e5 + 5));

// finish time array
vector<int> finish(((int)1e5 + 5));

// visited array, initially set to -1
vector<int> visited(((int)1e5 + 5), -1);
...
class Task {
 ...
 private:
	...
	void print_output(ifstream &fin, ofstream &fout) {
		for (int i = 0; i < Q; ++i) {
			int D;  // D = starting node
			int E;  // E = number of consecutive dispatches
			fin >> D >> E;

			// number of children of D
